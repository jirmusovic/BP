...
Nakonec bylo získáno 149 vyplněných a odeslaných dotazník\r u od respondent\r u. Dotazník pracuje s celými hodnotami 1-10, které jsem poté pro lepší práci s daty převedla na interval [0,1].

\subsection{Experiment zjiš\v tování, zda lidé chápou implikace jako ekvivalence}
Implikace i ekvivalence jsou v běžném jazyce lidmi hojně využívány. Problém při využívání těchto logických spojek však nastává při převodu běžného jazyka do matematiky a naopak a v jejich obecném chápání lidmi. Lidské chápání logických spojek, především pak implikace, je velmi rozmanité a matematicky nepřesné. Mějme výrok: \textit{\clqq Pokud venku prší, zmoknu.\crqq } Takový výrok je zřejmě implikací. Obecně je tato věta v běžném jazyce chápána poměrně jednoznačně. V běžném jazyce ale nastává problém takový, že tento výrok m\r uže být chápán jako ekvivalence, protože pokud někdo tvrdí, že zmokl, venku dozajista pršelo. Stává se tedy, že lidé zamě\v nují implikaci za ekvivalenci, protože klasické porozumění implikace přesně neodpovídá její matematické definici.

Jako další příklad bych ráda uvedla výrok: \textit{\clqq Když bude dnes svítit slunce, p\r ujdu do práce.\crqq } Tato věta je kupodivu pravdivá, pokud slunce nesvítí, ale i přesto jde člověk do práce. Také může být pravdivá, i když dotyčný nepůjde do práce – stačí, aby nesvítilo slunce, a podle prvního řádku pravdivostní tabulky bude mít stejně pravdu. Takovými zp\r usoby však není obecně na pravdivost výrok\r u nahlíženo. Pokud by slunce nesvítilo, ale daný člověk by stejně šel do práce, ostatní lidé by na něj spíše nahlíželi jako na lháře, i když podle definice implikace je jeho tvrzení stále pravdivé.\\
...
Z dat získaných pomocí dotazníku popsaného výše filtruji pomocí Python skriptu hodnoty získaných z výsledku provedením implikace $A \to B$ a poté $B \to A$. Ty následně porovnávám a pokud se výsledné hodnoty liší o jeden, respektive dva stupně osy, respondent nahlíží na výroky jako na ekvivalence. Takový proces jsem použila pro každou skupinu výrok\r u a výsledky na závěr porovnala. Výsledky zobrazuji v koláčových grafech pro zvýšení přehlednosti, kde modrá barva zastává odlišné hodnoty výsledk\r u a oranžová hodnoty stejné (nebo velmi blízké), tedy ekvivalenci. 
...
\begin{graph}
Koláčové grafy reprezentující shodu ve výrocích $A \to B$ a $B \to A$ v první skupině výrok\r u s odchylkou jednoho, respektive dvou díl\r u osy.
    \begin{figure}[H]
...
\begin{graph}
Koláčové grafy reprezentující shodu ve výrocích $A \to B$ a $B \to A$ ve druhé skupině výrok\r u s odchylkou jednoho, respektive dvou díl\r u osy.
    \begin{figure}[H]
...
\begin{graph}
Koláčové grafy reprezentující shodu ve výrocích $A \to B$ a $B \to A$ ve třetí skupině výrok\r u s odchylkou jednoho, respektive dvou díl\r u osy.
    \begin{figure}[H]
...
\begin{graph}
Koláčové grafy reprezentující shodu ve výrocích $A \to B$ a $B \to A$ ve čtvrté skupině výrok\r u s odchylkou jednoho, respektive dvou díl\r u osy.
    \begin{figure}[H]
...
\begin{graph}
Koláčové grafy reprezentující shodu ve výrocích $A \to B$ a $B \to A$ v páté skupině výrok\r u s odchylkou jednoho, respektive dvou díl\r u osy.
    \begin{figure}[H]
