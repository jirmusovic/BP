...
Za předpokladu, že je $\varphi : [0,1] \rightarrow [0,1]$ rostoucí bijekce, $T: [0,1]^2 \rightarrow [0,1]$ je t-norma a $T_{\varphi}$ je také t-norma, platí pak tvrzení $\varphi(T_{\varphi}(x, y)) = T(\varphi(x),\varphi(y))$. To znamená, že $\varphi$-transformace minimové t-normy $T_M$ je rovna $T_M$, což následující příklad \ref{sec: fi-transform} ukazuje.
\begin{example}
...
\end{tikzpicture}
\end{graph}
{\color{blue} obrazek nakreslim hned, jak budu mit graficky tablet (25.12), aby to nejak vypadalo}
Z obrázku je pak zřejmé, že tvoří $\varphi$-transformace jistý patern, který je možný pozorovat. Jednoduše řečeno, pokud je $\varphi$-transformace t-normy taktéž t-norma, graficky vytvořená \clqq L-ka\crqq \space musejí střídat své hodnoty, v tomto případě konkrétně: t-norma | konstanta | t-norma, atd.
...
\label{ex: fi-transform conorm}
    ...
    2x - 1, & \mbox {pokud } x \in [\frac{2}{3}, 1],
    \end{cases}$$
    pak lze zkoumat pr\r uběh stejně jako v příkladě \ref{sec: fi-transform}.
 Pokud by se tento graf rozd\v elil na sekce, kter\' e je pot\v reba samostatn\v e zkoumat - v tomto konkr\' etn\' im p\v r\' ipad\v e na t\v retiny - lze pak jednodu\v se vyjárřit, kter\' a \v cast grafu bude po $ \varphi - transformaci$ t-konormou a  kter\' a bude naopak nab\' yvat jin\' ych hodnot.
...
\begin{example}
    ...
    \begin{graph}{\color{blue}opět bude dokresleno - obrácená L-ka max/1/3/max}\\
...
\begin{example}
    ...
    \begin{graph}Lka ted budou jakoby o 90 stupnu obracena \\
