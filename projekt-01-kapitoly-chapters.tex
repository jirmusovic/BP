...
\section{Modelování fuzzy implikace}
...
Základní funkce $B-$spline řádu \textit{k} jsou definovány rekurzivně: \cite{Beliakov} \\
\begin{itemize}
    \item Pro k = 1:
\[
B_{i,1}(1-x) = 
...
\section{Aproximační algoritmus}
...
%\subsection{Výsledný aproximační algoritmus}
...
a:\\
$-4.96124x^2+5.41040x-0.42921$\\
$-0.23295x^2-0.10243x+0.99083$\\
...
\begin{tikzpicture}
        \begin{axis}[
            ...
            view={15}{30}, % Náhled na graf (úhel pohledu)
            legend style={at={(0.5,-0.2)}, anchor=north}
        ]
        
     \addplot3 [surf, shader=interp, mesh] {1-(-0.23295*((-4.96124*x^2+5.41040*x-0.42921) + -4.96124*(1-y)^2+5.41040*(1-y)-0.42921)^2-0.10243*((-4.96124*x^2+5.41040*x-0.42921) + -4.96124*(1-y)^2+5.41040*(1-y)-0.42921)+0.99083)};
        \legend{$I_{1a}$}
        \end{axis}
    \end{tikzpicture}\\
    
\begin{figure}[H]
    \caption{Generátor zkonstruován nad 1. skupinou při $A \to B$ a k němu náležící $(S,N)-$implikace}
        \hspace{-1cm}
        \includegraphics[scale=0.5]{template-fig/p0.pdf}
        %\includegraphics[scale=0.7]{template-fig/I1a.PNG}
        \centering
\end{figure}
...
Program obsahuje funkce \textit{BSplineEval} pro rekurzivní výpočet $B-$spline funkce v bodě \textit{x}, s stupněm \textit{k}, indexem \textit{i} a uzly \textit{t}, \textit{CalcPoints}, která provádí výpočty pomocí $B-$spline interpolace na datech z \textit{DataFrame} a funkce pro vykreslování a získávání předpis\r u pro výsledné generátory.
...
\begin{algorithm}[H]
 ...
 \SetNlSkip{-1.2em}
 \SetInd{1em}{1em}
 \Input{Množina empirických dat ve tvaru $\{\mathbf{x_1}^k, \mathbf{x_2}^k, y^k\}, k~= 1,2,\dots,n$}
 \Output{Vypočítané hodnoty bod\r u}
 \BlankLine
 \Indentp{1.7em}
...
Načítání dat pak probíhá opět ze souboru \textit{data.csv}. Data se zpracují podle výše popsaného pravidla na hodnoty $0-1$ a je pak s nimi dále pracováno. Výstupem celého programu je předpis a graf aproximovaného generátoru a jeho inverzní funcke.
