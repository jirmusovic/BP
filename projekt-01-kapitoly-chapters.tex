...
Generátory nespojitých t-norem tedy zřejmě nemusí být bijekce. Na následujícím příkladu je obdobně, jako v Příkladu 6. ukázka zevšeobecnění inverzní funkce. {\color{blue}dodelat odkaz}
\begin{example}
\cite{hlinena}
\begin{enumerate}
...
Na Obrázku 7. lze zřejmě pozorovat následující situaci: $$f_2(x)=f_3^{(-1)}\text{ a } f_3(x)=f_2^{(-1)}(x), \text{ takže } (f_2^{(-1)})^{(-1)}(x)=f_2(x) \text{ a }
            (f_3^{(-1)})^{(-1)}(x)=f_3(x).$$  {\color{blue}dodelat odkaz}
...
\begin{tikzpicture}
     ...
     \addplot3 [surf, shader=interp, mesh] {ifthenelse(min(1-x,x)==0,max(1-x,x),1)};
        \legend{$I_D$}
        \end{axis}
    \end{tikzpicture}
\end{graph}
...
\end{example}
\begin{graph}
     Grafické znáyornění R-implikátor\r u.\\
\begin{tikzpicture}
        \begin{axis}[
            width=7cm,  % Nastavte požadovanou šířku grafu
            height=7cm, % Nastavte požadovanou výšku grafu
...
{\color{blue} tady bych ta slova 'pokud' nechala, ale muzeme se o tom jeste pobavit}
\begin{definition}
\cite{hlinena}
